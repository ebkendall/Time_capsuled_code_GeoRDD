...
for(k in 1:n_buff_width) {
    ...
    for(ii in indexList_MAIN) {
        ...
        for(kk in 1:length(adjust_val)) {
            # abs(combinedMatchingSetupFix2$n_arr_1 / combinedMatchingSetupFix2$n_off_1
            #     - combinedMatchingSetupFix2$n_arr_2 / combinedMatchingSetupFix2$n_off_2)
            null_dist_int = abs(t_stat_int_surface_ii[w50, 3*kk-2] / t_stat_off_surface_ii[w50, 3*kk-2]
                                - t_stat_int_surface_ii[w50, 3*kk-1] / t_stat_off_surface_ii[w50, 3*kk-1])
